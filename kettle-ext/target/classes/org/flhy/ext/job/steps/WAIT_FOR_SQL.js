JobEntryWaitForSQLDialog = Ext.extend(KettleDialog, {
	width: 450,
	height: 600,
	autoScroll: true,
	title: '等待SQL',
	initComponent: function() {
		...
		var wRowsCountValue = new Ext.form.TextField({fieldLabel: '数值',anchor: '-10', value: cell.getAttribute('rowsCountValue')});
		var wMaximumTimeout = new Ext.form.TextField({fieldLabel: '最大超时时间',anchor: '-10', value: cell.getAttribute('maximumTimeout')});
		var wCheckCycleTime = new Ext.form.TextField({fieldLabel: '循环检查时间间隔',anchor: '-10', value: cell.getAttribute('checkCycleTime')});
		var wSuccessOnTimeout = new Ext.form.Checkbox({fieldLabel: '超时时成功',anchor: '-10',	disabled:true, checked: cell.getAttribute('successOnTimeout')== 'Y'});
		...
		var form = new Ext.form.FormPanel({
			...
			items: [{xtype: 'compositefield',
				...
				labelAlign: 'right',
				labelWidth: 130,
				items: [wSuccessCondition, wRowsCountValue,wMaximumTimeout,wCheckCycleTime,wSuccessOnTimeout]
			},{
			xtype: 'fieldset',
			title: '自定义SQL',
			items: [{layout: 'form', 
		...
		this.getValues = function(){
			return {
				...
				rowsCountValue : wRowsCountValue.getValue()   ,
				maximumTimeout : wMaximumTimeout.getValue()   ,
				checkCycleTime : wCheckCycleTime.getValue()   ,
				successOnTimeout : wSuccessOnTimeout.getValue()   ? "Y" : "N"   ,
...
//		this.fitItems = [form];
		...
		JobEntryWaitForSQLDialog.superclass.initComponent.call(this);
	},
